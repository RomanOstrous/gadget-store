...
export const ProductPage = () => {
  const { loading, error, product } = useContext(StorContext);
  ...
  const showPagination =
    perPage !== count && lastPage > 1 && searchParams.get('perPage') !== 'All';

  const start = currentPage * perPage - perPage;
  const end = currentPage * perPage <= count ? currentPage * perPage : count;
  ...
  return (
    <>
      <div className="product">
        {query && <SearchWindow product={filteredProducts} />}
        ...
        {!query && (
          <>
            <div className="product__top">
              ...
              <p className="product__count">{count} models</p>
            </div>
